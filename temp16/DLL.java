...
public class DLL<E extends Comparable<E>> {
   ...
   public int partition() {//Integer[] a
      ...
      while (loIndex < hiIndex){
         while (p.compareTo(hi.element) <= 0/*a[hi]*/ && loIndex != hiIndex){
            hi = hi.prev;
            hiIndex--;
         }
         if (hi != lo) {
            lo.element/*a[lo]*/ = hi.element/*a[hi]*/;
            loIndex++;
            lo = lo.next;
         }
         while (p.compareTo(lo.element) >= 0/*a[lo]*/ && loIndex != hiIndex){
            lo = lo.next;
            loIndex++;
         }
         if (hi != lo) {
            hi.element/*a[hi]*/ = lo.element/*a[lo]*/;
            hiIndex--;
            hi = hi.prev;
         }
      }
      hi.element/*a[hi]*/ = p;
      return hiIndex; // the pivot position
   }
